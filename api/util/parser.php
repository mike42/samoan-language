	...
	public function getInternalLinkInfo($info) {
		switch($info['namespace']) {
			case 'word':
				...
				$info['url'] = core::constructURL('word', 'view', array($info['target']), 'html');
				if(trim($info['target']) == '') {
				...
				return $newinfo;
			case 'ex':
				$info['url'] = core::constructURL('example', 'view', array($info['target']), 'html');
				break;
			default:
				if($info['namespace'] == "") {
					...
					$info['url'] = core::constructURL('page', 'view', array($info['title']), 'html');
				}
		}
		return $info;
	}
