...
class Page_Model implements Model {
	...
	public static function getByShort($id) {
		...
		if ($row = self::$database -> retrieve ( $query, 1, $id )) {
			$page = self::$database -> row_from_template ( $row, Page_Model::$template );
			$page ['page_rel_revision'] = self::$database -> row_from_template ( $row, Revision_Model::$template );
			return $page;
		} else {
			return false;
		}
	...
	public static function insert($short) {
		...
		self::$database -> retrieve ( $query, 0, $short );
	...
	public static function setCurrentRevisionID($page_id, $revision_id) {
		...
		self::$database -> retrieve ( $query, 0, $revision_id, $page_id );
	}
	...
	public static function delete($id) {
		...
		self::$database -> retrieve ( $query, 0, ( int ) $id );
